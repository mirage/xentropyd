...
module Connection = struct
  ...
  let to_assoc_list t =
    ...
    let frontend_acl =
      Xenstore.Protocol.ACL.({owner = t.frontend_domid; other = NONE; acl = [ t.backend_domid, READ ]}) in
    let backend_acl =
      Xenstore.Protocol.ACL.({owner = t.backend_domid; other = NONE; acl = [ t.frontend_domid, READ ]}) in
